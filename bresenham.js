...
// Slight edits to original code, such as having setPixel* be parameter 
// instead of undeclared funtion.
...
function plotLine(setPixel, x0, y0, x1, y1)
{
...
function plotEllipse(setPixel, xm, ym, a, b)
{
...
function plotCircle(setPixel, xm, ym, r)
{
...
function plotEllipseRect(setPixel, x0, y0, x1, y1)
...
function plotQuadBezierSeg(setPixel, x0, y0, x1, y1, x2, y2)
{                                  /* plot a limited quadratic Bezier segment */
  ...
  plotLine(setPixel, x0,y0, x2,y2);                       /* plot remaining part to end */
}

function plotQuadBezier(setPixel, x0, y0, x1, y1, x2, y2)
{                                          /* plot any quadratic Bezier curve */
   ...
   if (x*(x2-x1) > 0) {                              /* horizontal cut at P4? */
      ...
      plotQuadBezierSeg(setPixel, x0,y0, x,Math.floor(r+0.5), x,y);
   ...
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      plotQuadBezierSeg(setPixel, x0,y0, Math.floor(r+0.5),y, x,y);
      r = (x1-x2)*(t-y2)/(y1-y2)+x2;                  /* intersect P7 | P1 P2 */
      x0 = x; x1 = Math.floor(r+0.5); y0 = y1 = y;        /* P0 = P6, P1 = P7 */
   }
   plotQuadBezierSeg(setPixel, x0,y0, x1,y1, x2,y2);                  /* remaining part */
}

function plotQuadRationalBezierSeg(setPixel, x0, y0, x1, y1, x2, y2, w)
{                   /* plot a limited rational Bezier segment, squared weight */
  ...
  if (cur != 0.0 && w > 0.0) {                            /* no straight line */
    ...
    if (w < 0.5 && (dy > xy || dx < xy)) {   /* flat ellipse, algorithm fails */
       ...
       plotQuadRationalBezierSeg(setPixel, x0,y0, dx,dy, sx,sy, cur);/* plot separately */
       dx = Math.floor((w*x1+x2)*xy+0.5); dy = Math.floor((y1*w+y2)*xy+0.5);
       plotQuadRationalBezierSeg(setPixel, sx,sy, dx,dy, x2,y2, cur);
       return;
    }
  ...
  plotLine(setPixel, x0,y0, x2,y2);                     /* plot remaining needle to end */
}

function plotQuadRationalBezier(setPixel, x0, y0, x1, y1, x2, y2, w)
{                                 /* plot any quadratic rational Bezier curve */
   ...
   if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
      ...
      plotQuadRationalBezierSeg(setPixel, x0,y0, x,Math.floor(yy+0.5), x,y, ww);
   ...
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      plotQuadRationalBezierSeg(setPixel, x0,y0, Math.floor(xx+0.5),y, x,y, ww);
      xx = (x1-x2)*(yy-y2)/(y1-y2)+x2;                /* intersect P7 | P1 P2 */
      x1 = Math.floor(xx+0.5); x0 = x; y0 = y1 = y;       /* P0 = P6, P1 = P7 */
   }
   plotQuadRationalBezierSeg(setPixel, x0,y0, x1,y1, x2,y2, w*w);          /* remaining */
}

function plotRotatedEllipse(setPixel, x, y, a, b, angle)
{                                   /* plot ellipse rotated by angle (radian) */
   ...
   plotRotatedEllipseRect(setPixel, x-a,y-b, x+a,y+b, (4*zd*Math.cos(angle)));
}

function plotRotatedEllipseRect(setPixel, x0, y0, x1, y1, zd)
{                  /* rectangle enclosing the ellipse, integer rotation angle */
   var xd = x1-x0, yd = y1-y0, w = xd*yd;
   if (zd == 0) return plotEllipseRect(setPixel, x0,y0, x1,y1);          /* looks nicer */
   ...
   plotQuadRationalBezierSeg(setPixel, x0,y0+yd, x0,y0, x0+xd,y0, 1.0-w);
   plotQuadRationalBezierSeg(setPixel, x0,y0+yd, x0,y1, x1-xd,y1, w);
   plotQuadRationalBezierSeg(setPixel, x1,y1-yd, x1,y1, x1-xd,y1, 1.0-w);
   plotQuadRationalBezierSeg(setPixel, x1,y1-yd, x1,y0, x0+xd,y0, w);
}

function plotCubicBezierSeg(setPixel, x0, y0, x1, y1, x2, y2, x3, y3)
{                                        /* plot limited cubic Bezier segment */
   ...
   if (xa == 0 && ya == 0)                                /* quadratic Bezier */
      return plotQuadBezierSeg(setPixel, x0,y0, (3*x1-x0)>>1,(3*y1-y0)>>1, x3,y3);
...
exit: 
   ...
   plotLine(setPixel, x0,y0, x3,y3);       /* remaining part in case of cusp or crunode */
}

function plotCubicBezier(setPixel, x0, y0, x1, y1, x2, y2, x3, y3)
{                                              /* plot any cubic Bezier curve */
   ...
   for (i = 0; i <= n; i++) {                 /* plot each segment separately */
      ...
      if (x0 != x3 || y0 != y3)                            /* segment t1 - t2 */
         plotCubicBezierSeg(setPixel, x0,y0, x0+fx1,y0+fy1, x0+fx2,y0+fy2, x3,y3);
